...
$(document).ready(function () {
    ...
    // 페이지가 로드된 후 쿼리 파라미터 확인 및 모달 표시
    var urlParams = new URLSearchParams(window.location.search);
    var requestIdParam = urlParams.get('requestId');
    if (requestIdParam) {
        var requestTitleElement = $(".myRequest_list_title[data-request-id='" + requestIdParam + "']");
        if (requestTitleElement.length) {
            requestTitleElement.click();
        }
    }
});
